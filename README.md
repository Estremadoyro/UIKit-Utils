...
Should be called inside **viewDidLayoutSubviews** in a VC, but if not, an **async** call will do it in a **UIView** subclass. **IMPORTANT**, *viewDidLayoutSubviews* can be called multiple times without the subviews updating, hence you **must** prevent the *shadowLayer* from getting called endlessly by any layout update. Just call it once, otherwise multiple shadows will stack on top of each other and consume memory significally. You could make the CAShapeLayer optional and only run the code when it's *nil* this way it will be called only once per lifecycle. However, there are multiple ways to accomplish this same behavior, implement them as convenient.
```swift
    DispatchQueue.main.async { [weak self] in
